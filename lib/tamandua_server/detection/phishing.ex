defmodule TamanduaServer.Detection.Phishing do
  ...
  def analyze(email_data) do
    GenServer.call(__MODULE__, {:analyze, email_data}, 120_000)
  end
  ...
  def get_report(report_id) do
    case :ets.lookup(@ets_submissions, report_id) do
      [{^report_id, report}] -> {:ok, report}
      [] -> {:error, :not_found}
    end
  rescue
  ...
  def list_campaigns(opts \\ []) do
    ...
    campaigns =
      ...
      |> Enum.map(fn {_id, campaign} -> campaign end)
  ...
  def report_phish(params) do
    GenServer.call(__MODULE__, {:report_phish, params}, 120_000)
  end
  ...
  def handle_call(:get_stats, _from, state) do
    submissions_count =
      try do
        :ets.info(@ets_submissions, :size) || 0
      rescue
        _ -> 0
      end
    ...
    campaigns_count =
      try do
        :ets.info(@ets_campaigns, :size) || 0
      rescue
        _ -> 0
      end

    reply = Map.merge(state.stats, %{
      cached_submissions: submissions_count,
  ...
  defp do_analyze(email_data) do
    ...
    if message_id && not is_nil(dedup_lookup(message_id)) do
      {:ok, dedup_lookup(message_id)}
    else
      with {:ok, parsed} <- parse_email(email_data),
        ...
        recommendations = build_recommendations(verdict, header_result, url_result, attach_result, sender_result)
        ...
        # Store in ETS
        :ets.insert(@ets_submissions, {report_id, report})

        if message_id do
          :ets.insert(@ets_submissions, {{:dedup, message_id}, report})
        end
  ...
  defp check_spf(parsed, findings) do
    ...
    spf_result = extract_auth_mechanism(auth_header, "spf") ||
                 extract_spf_from_header(spf_header)

    findings = case spf_result do
      "fail" ->
  ...
  defp analyze_received_chain(received_headers, findings) do
    # Detect unusual hops
    suspicious_hops = Enum.count(received_headers, fn header ->
    ...
    findings = if suspicious_hops > 0 do
      [{:suspicious_received, "#{suspicious_hops} suspicious hop(s) in Received chain", 15} | findings]
    else
      findings
    end

    # Detect large hop counts (possible relay chains)
    findings = if length(received_headers) > 8 do
      [{:many_hops, "Unusually large number of mail relays (#{length(received_headers)})", 10} | findings]
    else
      findings
    end
  ...
  defp check_x_mailer(parsed, findings) do
    ...
    match = Enum.find(@suspicious_mailers, fn tool ->
      String.contains?(x_mailer_lower, String.downcase(tool))
    end)

    if match do
      [{:suspicious_mailer,
        "X-Mailer header matches known phishing tool: #{match}", 50} | findings]
    else
      findings
    end
  end
  ...
  defp analyze_urls(parsed) do
    ...
    url_results = Enum.map(all_urls, &analyze_single_url/1)

    max_score = if Enum.empty?(url_results) do
      0
    else
  ...
  defp analyze_single_url(url) do
    ...
    {findings, score} = if is_shortener do
    ...
    {findings, score} = if homograph.detected do
    ...
    {findings, score} = if typosquat.detected do
    ...
    {findings, score} = if tld in @suspicious_tlds do
    ...
    # IP address URL
    {findings, score} = if ip_address?(host) do
    ...
    ioc_match = ioc_match?(url, host_lower)
    {findings, score} = if ioc_match do
  ...
  defp check_url_path(path, findings, score) do
    ...
    cond do
      ...
      Regex.match?(~r/(login|signin|verify|account|secure|update|confirm|password)/i, path_lower) ->
        {[{:credential_path, "URL path contains credential-harvesting keywords"} | findings], score + 20}
  ...
  defp detect_homograph(host) do
    ...
    has_confusables = Enum.any?(chars, fn char ->
  ...
  defp detect_typosquatting(domain) do
    ...
    match = Enum.find_value(@brand_domains, fn {brand, legit_domains} ->
    ...
    if match do
      ...
      %{detected: true, brand: brand, legitimate: legit, distance: dist,
  ...
  defp ioc_match?(url, host) do
    IOCs.match?(url, "url") || IOCs.match?(host, "domain")
  rescue
    _ -> false
  catch
    :exit, _ -> false
  ...
  defp analyze_attachments(parsed) do
    results = Enum.map(parsed.attachments, &analyze_single_attachment/1)

    max_score = if Enum.empty?(results) do
      0
    else
  ...
  defp analyze_single_attachment(att) do
    ...
    {findings, score} = if double_extension?(filename) do
    ...
    # Dangerous extension
    {findings, score} = if ext in @dangerous_extensions do
    ...
    {findings, score} = if ext in @macro_extensions do
    ...
    {findings, score} = if detected_type && content_type_mismatch?(ext, detected_type) do
    ...
    {findings, score} = check_hash_iocs(hashes, findings, score)

    # Password-protected archive detection
    {findings, score} = if ext in @archive_extensions and password_protected?(content) do
  ...
  defp check_magic_bytes(content, findings, score) when byte_size(content) < 2, do: {nil, findings, score}
  defp check_magic_bytes(content, findings, score) do
    detected = Enum.find_value(@magic_bytes, fn {magic, type} ->
      if binary_part_safe(content, 0, byte_size(magic)) == magic, do: type
    end)

    if detected == "application/x-dosexec" do
      {detected, [{:pe_executable, "File contains PE executable magic bytes"} | findings], score + 50}
    else
  ...
  defp detect_macro_content(content, ext, findings, score) do
    has_vba_marker = is_binary(content) and
    ...
    if has_vba_marker and ext not in @macro_extensions do
      {[{:hidden_macro, "VBA macro indicators found in non-macro file type"} | findings], score + 40}
    else
      {findings, score}
    end
  end
  ...
  defp check_hash_iocs(hashes, findings, score) when map_size(hashes) == 0, do: {findings, score}
  defp check_hash_iocs(hashes, findings, score) do
    match_found = Enum.any?([:sha256, :sha1, :md5], fn algo ->
      hash = Map.get(hashes, algo)
      hash && ioc_hash_match?(hash, algo)
    end)

    if match_found do
      {[{:known_malicious_hash, "Attachment hash matches known malicious IOC"} | findings], score + 90}
    else
      {findings, score}
    end
  end

  defp ioc_hash_match?(hash, algo) do
    IOCs.match?(hash, "hash_#{algo}")
  rescue
    _ -> false
  catch
    :exit, _ -> false
  ...
  defp analyze_sender(parsed) do
    ...
    rep = get_sender_reputation(email_addr)

    {findings, score} = case rep do
      %{malicious_ratio: r} when r > 0.5 ->
        {[{:bad_reputation, "Sender has high malicious email ratio (#{Float.round(r * 100, 1)}%)"} | findings], score + 50}
      %{first_time: true} ->
    ...
    {findings, score} = if display_name && String.contains?(display_name, "@") do
      {[{:display_name_email, "Display name contains an email address (spoofing indicator)"} | findings], score + 35}
    else
      {findings, score}
    end
    ...
    {findings, score} = if return_domain && domain && return_domain != domain do
      {[{:return_path_mismatch, "Return-Path domain (#{return_domain}) differs from sender"} | findings], score + 25}
    else
      {findings, score}
    end
    ...
    {findings, score} = if typo.detected do
    ...
    update_sender_reputation(email_addr)
  ...
  defp detect_bec_patterns(display_name, subject, findings, score) do
    ...
    cond do
      subject_match and name_match ->
        {[{:bec_pattern, "Business Email Compromise pattern: executive title + financial subject"} | findings], score + 50}
      subject_match ->
  ...
  defp detect_executive_impersonation(display_name, _email_addr, findings, score) do
    ...
    if display_name && length(known_executives) > 0 do
      ...
      match = Enum.find(known_executives, fn exec_name ->
      ...
      if match do
        {[{:executive_impersonation,
          "Display name closely resembles known executive: #{match}"} | findings], score + 55}
      else
        {findings, score}
      end
    else
  ...
  defp extract_domain(addr) do
    ...
    # Handle <addr> format
    clean = case Regex.run(~r/<([^>]+)>/, clean) do
      [_, inner] -> inner
      nil -> clean
    end
  ...
  defp get_sender_reputation(nil), do: %{first_time: true, malicious_ratio: 0.0}
  defp get_sender_reputation(email) do
    case :ets.lookup(@ets_sender_rep, email) do
      [{^email, rep}] ->
        # Check TTL
  ...
  defp update_sender_reputation(nil), do: :ok
  defp update_sender_reputation(email) do
    ...
    case :ets.lookup(@ets_sender_rep, email) do
      [{^email, existing}] ->
        ...
        :ets.insert(@ets_sender_rep, {email, updated})
      ...
      [] ->
        ...
        :ets.insert(@ets_sender_rep, {email, rep})
    end
  ...
  defp analyze_content(parsed) do
    ...
    {findings, score} = Enum.reduce(urgency_patterns, {findings, score}, fn {pattern, pts}, {f, s} ->
    ...
    {findings, score} = if Regex.match?(~r/password|credential|login|username|ssn|social\s*security|bank\s*account/i, body) do
      {[{:credential_request, "Request for sensitive information detected"} | findings], score + 25}
    else
      {findings, score}
    end

    # Generic greeting
    {findings, score} = if Regex.match?(~r/^(dear\s+(customer|user|member|sir|madam|valued)|hello,?\s*$)/im, body) do
      {[{:generic_greeting, "Generic greeting used instead of personal name"} | findings], score + 10}
    else
      {findings, score}
    end

    # Hidden text in HTML
    {findings, score} = if parsed.html_body && hidden_text?(parsed.html_body) do
    ...
    {findings, score} = if parsed.html_body && String.contains?(parsed.html_body, "<form") do
    ...
    {findings, score} = if Regex.match?(~r/data:application\/(octet-stream|javascript)/i, body) do
  ...
  defp extract_all_iocs(parsed, url_result, attach_result) do
    ...
    hashes = attach_result.attachments
  ...
  defp compute_findings_score(findings, max_score) do
    total = Enum.reduce(findings, 0, fn
  ...
  defp correlate_campaign(parsed, url_result, attach_result, sender_result) do
    ...
    attach_hashes = attach_result.attachments
    ...
    if campaign do
      update_campaign(campaign.id, fingerprint)
      %{
  ...
  defp update_campaign(campaign_id, fingerprint) do
    case :ets.lookup(@ets_campaigns, campaign_id) do
      [{^campaign_id, campaign}] ->
        updated = %{campaign |
          ...
          status: if(campaign.email_count + 1 >= @campaign_min_emails, do: "active", else: "pending")
        }
        :ets.insert(@ets_campaigns, {campaign_id, updated})
  ...
  defp register_campaign_candidate(fingerprint) do
    ...
    :ets.insert(@ets_campaigns, {id, campaign})
  rescue
    _ -> :ok
  end
  ...
  defp sweep_campaigns do
    ...
    :ets.tab2list(@ets_campaigns)
    |> Enum.each(fn {id, campaign} ->
      if DateTime.compare(campaign.last_seen, cutoff) == :lt do
        :ets.delete(@ets_campaigns, id)
      end
    end)
  rescue
    _ -> :ok
  ...
  defp build_recommendations(verdict, header_result, url_result, attach_result, sender_result) do
    base = case verdict do
      :spear_phishing ->
        [
          %{action: :quarantine, priority: :critical,
            reason: "Targeted spear-phishing detected -- quarantine immediately"},
          %{action: :block_sender, priority: :high,
            reason: "Block sender domain and add IOCs to blocklists"},
          %{action: :notify_targets, priority: :high,
            reason: "Alert all targeted recipients"},
      ...
      :phishing ->
        [
          ...
          %{action: :block_sender, priority: :medium,
            reason: "Consider blocking sender domain"},
          %{action: :check_spread, priority: :medium,
            reason: "Check if other users received similar emails"}
        ]

      :suspicious ->
        [
          %{action: :hold, priority: :medium,
            reason: "Suspicious email -- hold for manual review"},
          %{action: :sandbox_urls, priority: :medium,
            reason: "Detonate URLs in sandbox before releasing"},
          %{action: :verify_sender, priority: :low,
            reason: "Verify sender through out-of-band communication"}
        ]

      :clean ->
        [
          %{action: :allow, priority: :low,
            reason: "Email appears safe -- allow delivery"},
          %{action: :add_allowlist, priority: :low,
    ...
    extras = if url_result.shorteners_found > 0 do
      [%{action: :resolve_shorteners, priority: :medium,
         reason: "Resolve #{url_result.shorteners_found} shortened URL(s) before releasing"} | extras]
    else
      extras
    end

    extras = if attach_result.total > 0 and attach_result.score >= 40 do
      [%{action: :sandbox_attachments, priority: :high,
         reason: "Submit #{attach_result.total} attachment(s) for sandbox detonation"} | extras]
    else
      extras
    end

    extras = if header_result.spf == "fail" or header_result.dmarc == "fail" do
      [%{action: :auth_failure, priority: :high,
         reason: "Email authentication failed -- sender may be spoofed"} | extras]
    else
      extras
    end

    extras = if sender_result.typosquat.detected do
      [%{action: :typosquat_alert, priority: :high,
         reason: "Sender domain appears to be typosquatting #{sender_result.typosquat.brand}"} | extras]
    else
      extras
    end
  ...
  defp maybe_create_alert(report, email_data) do
    severity = case report.verdict do
    ...
    techniques = case report.verdict do
    ...
    Alerts.create_alert(%{
      ...
      evidence: %{
        ...
        network: Enum.map(report.iocs.urls, fn url ->
          %{type: "url", value: url, direction: "inbound"}
        end),
        file_hashes: Enum.map(report.iocs.file_hashes, fn h ->
          %{hash: h, type: "sha256"}
        end)
      },
      metadata: %{
  ...
  defp update_stats(state, report) do
    ...
    campaigns_detected = try do
      :ets.tab2list(@ets_campaigns)
      |> Enum.count(fn {_, c} -> c.status == "active" end)
    rescue
  ...
  defp broadcast_analysis(report) do
    Phoenix.PubSub.broadcast(@pubsub, @topic, {:phishing_analysis_complete, %{
      id: report.id,
  ...
  defp dedup_lookup(message_id) do
    case :ets.lookup(@ets_submissions, {:dedup, message_id}) do
      [{{:dedup, _}, report}] -> report
      [] -> nil
    end
  rescue
    _ -> nil
