defmodule TamanduaServerWeb.GraphQL.Resolvers.AlertResolver do
  ...
  alias TamanduaServer.{Alerts, Agents, Accounts, Repo}
  ...
  def alert_stats(_parent, _args, %{context: context}) do
    ...
    new_count = base_query
    |> where([a], a.status == "new")
    |> Repo.aggregate(:count)

    investigating_count = base_query
    |> where([a], a.status == "investigating")
    |> Repo.aggregate(:count)

    resolved_count = base_query
  ...
  def assigned_to(alert, _args, _resolution) do
    if alert.assigned_to_id do
      {:ok, Accounts.get_user(alert.assigned_to_id)}
    else
      {:ok, nil}
    end
  end

  def related_events(alert, args, _resolution) do
    limit = args[:limit] || 20
    ...
    error ->
      Logger.warning("related_events resolver failed for alert=#{inspect(alert.id)}: #{inspect(error)}")
      {:ok, []}
  end

  def timeline(alert, _args, _resolution) do
    ...
    # Add related events
    event_ids = alert.event_ids || []
    unless Enum.empty?(event_ids) do
      events = from(e in Event,
        where: e.id in ^event_ids,
      ...
      event_timeline = Enum.map(events, fn e ->
        %{
      ...
      timeline_events = timeline_events ++ event_timeline
    end
  ...
  def update_alert(_parent, %{id: id, input: input}, %{context: context}) do
    ...
    case Repo.one(query) do
      ...
      alert ->
        attrs = Map.take(input, [:status, :severity, :resolution_notes, :assigned_to_id])
