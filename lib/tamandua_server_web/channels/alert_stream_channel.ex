defmodule TamanduaServerWeb.AlertStreamChannel do
  @moduledoc """
  WebSocket channel for streaming alerts.

  Topic: "stream:alerts"

  Join with authentication token and optional filters:
  ```
  channel.join("stream:alerts", {
    token: "jwt_token",
    filters: {
  ...
  Pushed events:
  - "alert" - New alert matching filters
  ...
  @impl true
  def join("stream:alerts", params, socket) do
    ...
    unless socket.assigns[:current_user] do
      ...
      filters = Map.put(filters, :stream_type, [:alert])
      ...
      # Generate stream ID
      stream_id = generate_stream_id(socket, "alerts")
      ...
      socket = assign(socket, :alert_count, 0)

      Logger.info("Alert stream joined: #{stream_id} (user: #{socket.assigns.user_id})")
  ...
  @impl true
  def handle_info({:stream_data, :alert, data}, socket) do
    # Format and push alert to client
    formatted_data = format_data(data, socket.assigns.options)

    push(socket, "alert", %{
      id: socket.assigns.alert_count + 1,
      data: formatted_data,
      timestamp: System.system_time(:millisecond)
    })

    # Update alert count
    socket = assign(socket, :alert_count, socket.assigns.alert_count + 1)
  ...
  def handle_in("update_filters", %{"filters" => new_filters}, socket) do
    ...
    Logger.info("Alert stream filters updated: #{stream_id}")
  ...
  def terminate(reason, socket) do
    ...
    if stream_id do
      StreamManager.unregister_stream(stream_id)
      Logger.info("Alert stream terminated: #{stream_id} (reason: #{inspect(reason)})")
    end
    :ok
  end
  ...
  defp parse_filters(params) do
    ...
    filters = maybe_add_filter(filters, :status, params["status"])
