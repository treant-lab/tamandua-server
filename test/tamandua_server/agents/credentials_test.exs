defmodule TamanduaServer.Agents.CredentialsTest do
  ...
  describe "issue_credential/3" do
    ...
    test "stores IP address when provided", %{agent: agent, org: org} do
      {:ok, _jti, credential} = Credentials.issue_credential(
        agent.id, org.id,
  ...
  describe "validate_and_record_use/4" do
    test "validates and updates usage for valid credential", %{agent: agent, org: org} do
      ...
      assert {:ok, credential} = Credentials.validate_and_record_use(jti, agent.id, org.id, "10.0.0.1")
  ...
  describe "valid?/1" do
    test "returns true for valid credential", %{agent: agent, org: org} do
      ...
      assert Credentials.valid?(jti) == true
    end

    test "returns false for non-existent credential" do
      assert Credentials.valid?("nonexistent") == false
    end

    test "returns false for revoked credential", %{agent: agent, org: org} do
      {:ok, jti, _} = Credentials.issue_credential(agent.id, org.id)
      {:ok, _} = Credentials.revoke(jti, "test")

      assert Credentials.valid?(jti) == false
    end
  end
  ...
  describe "revoke/2" do
    test "revokes a credential", %{agent: agent, org: org} do
      ...
      assert {:ok, revoked} = Credentials.revoke(jti, "test_reason")
    ...
    test "returns error for non-existent credential" do
      assert {:error, :not_found} = Credentials.revoke("nonexistent", "test")
    end
  end

  describe "revoke_all_for_agent/2" do
    test "revokes all credentials for an agent", %{agent: agent, org: org} do
      {:ok, jti1, _} = Credentials.issue_credential(agent.id, org.id)
      {:ok, jti2, _} = Credentials.issue_credential(agent.id, org.id)
      {:ok, jti3, _} = Credentials.issue_credential(agent.id, org.id)

      assert {:ok, 3} = Credentials.revoke_all_for_agent(agent.id, "bulk_revoke")

      # All should be invalid now
      assert Credentials.valid?(jti1) == false
      assert Credentials.valid?(jti2) == false
      assert Credentials.valid?(jti3) == false
    end
    ...
    test "does not affect already-revoked credentials", %{agent: agent, org: org} do
      ...
      # Revoke one first
      {:ok, _} = Credentials.revoke(jti1, "first_revoke")

      # Now bulk revoke - should only affect the one remaining
      assert {:ok, 1} = Credentials.revoke_all_for_agent(agent.id, "bulk_revoke")
    end

    test "returns 0 when no active credentials exist", %{agent: agent} do
      assert {:ok, 0} = Credentials.revoke_all_for_agent(agent.id, "nothing_to_revoke")
    end
  end
  ...
  describe "list_active_for_agent/1" do
    test "returns only active credentials", %{agent: agent, org: org} do
      ...
      active = Credentials.list_active_for_agent(agent.id)
  ...
  describe "get_stats/1" do
    test "returns correct statistics", %{agent: agent, org: org} do
      ...
      stats = Credentials.get_stats(agent.id)
  ...
  describe "cleanup_expired/1" do
    test "removes old expired credentials", %{agent: agent, org: org} do
      ...
      assert {:ok, 1} = Credentials.cleanup_expired(30)
    ...
    test "does not remove recent expired credentials", %{agent: agent, org: org} do
      ...
      assert {:ok, 0} = Credentials.cleanup_expired(30)
  ...
  describe "AgentCredential schema validation" do
    test "requires finite expiry", %{agent: agent, org: org} do
      ...
      changeset = AgentCredential.changeset(%AgentCredential{}, %{
        agent_id: agent.id,
